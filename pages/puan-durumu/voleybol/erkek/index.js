import Layout from"../../../../components/layout";import Image1 from"next/image";import AdSense from "react-adsense";import Head from"next/head";import{Icon,Popup,Table}from"semantic-ui-react";import{Grid}from"semantic-ui-react";import{useEffect}from"react";import{absoluteUrl,getAppCookies,verifyToken}from"../../../../middleware/utils";import{useContext}from"react";import{AuthContext}from"../../../../contexts/AuthProvider";
export default function ErkekVoleybolPuanDurumu({ puanDurumu, profile }) {
 const{status:status,user:user}=useContext(AuthContext);useEffect(()=>{let leg;void 0!==profile.email?(user.setUserDetails({id:profile.id,email:profile.email,username:profile.username,firstname:profile.firstname,lastname:profile.lastname,roleId:profile.roleId,profile:profile.profile,isBanned:profile.isBanned}),status.setLoggedIn(!0)):status.setLoggedIn(!1)},[profile]);
  return (
    <Layout>
      <Head>
       <title>Fenerbahçe Erkek Voleybol Puan Durumu | Kanaryasokağı | Fenerbahçe haberlerinin tek adresi</title>
             <meta charSet="utf-8" />
          <meta name="viewport" content="width=device-width, initial-scale=1" />
			  <meta name="title" content="Fenerbahçe Erkek Voleybol Puan Durumu | Kanaryasokağı | Fenerbahçe haberlerinin tek adresi"/>
			<meta name="description" content="Fenerbahçe Erkek Voleybol Takımı'nın En Güncel Puan Durumu. "/>
			<meta name="keywords" content="kanaryasokağı, fenerbahçe Erkek Voleybol, fenerbahçe Voleybol, fenerbahçe, fenerbahçe Puan Durumu, fenerbahçe Voleybol Puan Durumu"/>
			<meta name="revisit-after" content="3 days"/>
          <link rel="icon" href="/kanaryasokagi.ico" />
		  <link rel="apple-touch-icon" href="/kanaryasokagi.ico"/>
	  <link rel="canonical" href="https://kanaryasokagi.com/puan-durumu/voleybol/erkek"/>
      </Head>
              ...
              <Table celled unstackable selectable>
                <Table.Header className="xxs:text-2xs xs:text-xxs sm:text-xs md:text-sm lg:text-tiny xl:text-base 2xl:text-lg">
                  <Table.Row>
                    ...
                    <Table.HeaderCell textAlign="center">
                      <Popup content="Alınan Set" trigger={<span>AS</span>} />
                    </Table.HeaderCell>
                    <Table.HeaderCell textAlign="center">
                      <Popup content="Verilen Set" trigger={<span>VS</span>} />
                    </Table.HeaderCell>
                    <Table.HeaderCell textAlign="center">
                      <Popup content="Set Averajı" trigger={<span>SAV</span>} />
                    </Table.HeaderCell>
                    <Table.HeaderCell textAlign="center">
                      <Popup
                        content="Atılan Sayı"
                        trigger={<Icon name="volleyball ball" color="green" />}
                      />
                    </Table.HeaderCell>
                    <Table.HeaderCell textAlign="center">
                      <Popup
                        content="Yenilen Sayı"
                        trigger={<Icon name="volleyball ball" color="red" />}
                      />
                    </Table.HeaderCell>
                  ...
                  {puanDurumu.data.map((takim, index) => (
                    <Table.Row className="font-poppins" key={index}>
                      <Table.Cell textAlign="center">
                        ...
                        {index > 3 && index < 8 && (
                          <>
                            <Icon name="square" color="blue" size="tiny" />
                            <span className="sm:text-3xs md:text-2xs lg:text-xs xl:text-sm font-poppins tracking-tighter">
                              {index + 1}
                            </span>
                          </>
                      ...
                      <Table.Cell textAlign="center">{takim.averaj.toFixed(3)}</Table.Cell>
