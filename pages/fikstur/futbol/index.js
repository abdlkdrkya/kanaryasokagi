...
export default function FutbolFikstur({ fikstur,profile }) {
 const{status:status,user:user}=useContext(AuthContext);useEffect(()=>{let leg;void 0!==profile.email?(user.setUserDetails({id:profile.id,email:profile.email,username:profile.username,firstname:profile.firstname,lastname:profile.lastname,roleId:profile.roleId,profile:profile.profile,isBanned:profile.isBanned}),status.setLoggedIn(!0)):status.setLoggedIn(!1)},[profile]);
  return (
    <Layout>
      <Head>
         <title>Fenerbahçe Erkek Futbol Fikstur - Kanaryasokağı - Fenerbahçeye dair</title>
             <meta charSet="utf-8" />
          <meta name="viewport" content="width=device-width, initial-scale=1" />
			  <meta name="title" content="Fenerbahçe Erkek Futbol Fikstur - Kanaryasokağı - Fenerbahçeye dair"/>
			<meta name="description" content="Fenerbahçe Erkek Futbol Takımı'nın En Güncel Fiksturü. "/>
			<meta name="keywords" content="kanaryasokağı, fenerbahçe Erkek Futbol, fenerbahçe futbol, fenerbahçe, fenerbahçe fikstürü, fenerbahçe futbol fikstürü"/>
              ...
              <Table celled selectable>
                <Table.Header>
                  <Table.Row>
                    ...
                    <Table.HeaderCell textAlign="center">İlk Yarı</Table.HeaderCell>
                    <Table.HeaderCell textAlign="center">Maç Sonucu</Table.HeaderCell>
                  </Table.Row>
                </Table.Header>
                ...
                <Table.Body>
                  {fikstur.data.map((mac, index) => (
                    <Table.Row  className="font-poppins" key={index}>
                      <Table.Cell textAlign="center">{mac.fiksturTarih}</Table.Cell>
                      <Table.Cell textAlign="center">
                          ...
                          <span>{mac.fiksturEvSahibi}{" - "}{mac.fiksturDeplasman}</span>
                      ...
                      <Table.Cell textAlign="center">
                       {mac.fiksturEvSahibiIlkYariSkor}{" - "}{mac.fiksturDeplasmanIlkYariSkor}
                      </Table.Cell>
                      <Table.Cell textAlign="center">
                       {mac.fiksturEvSahibiSkor}{" - "}{mac.fiksturDeplasmanSkor}
                      </Table.Cell>
                    </Table.Row>
                  ))}
                </Table.Body>
