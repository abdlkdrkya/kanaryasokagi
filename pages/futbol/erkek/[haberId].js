              ...
              {puanDurumu.data.map((maclar, index) => (
                <SwiperSlide key={index}>
                  <div className="bg-white h-20 transition duration-300 ease-in-out transform hover:scale-90">
                    {index < 2 && <div className="w-full h-0.5 bg-green"></div>}
                    {index > 1 && index < 4 && (
                    ...
                    {index > 15 && <div className="w-full h-0.5 bg-red"></div>}
...
export async function getServerSideProps(context) {
  ...
  const [haberlerRes, puanDurumuRes, erkekFutbolcularRes, hRes, yorumlarRes] =
      await Promise.all([
        fetch(
          "http://localhost:8080/api/haberler/getByHaberKategoriAndCinsiyet?categoryId=2&cinsiyetId=0"
        ),
  ...
  haberler.data.map((haber) => {
    haber.haberId !== h.data.haberId && haberler2.push(haber);
  });
  return {
    props: {
