import Head from"next/head";import Layout from"../../components/layout";import{Grid,Image,Card}from"semantic-ui-react";import AdSense from "react-adsense";import{useState}from"react";import{Swiper,SwiperSlide}from"swiper/react";import Link from"next/link";import SwiperCore,{Autoplay,Navigation,Thumbs,Pagination,Keyboard}from"swiper";import{useContext,useEffect}from"react";import{AuthContext}from"../../contexts/AuthProvider";import{absoluteUrl,getAppCookies,verifyToken}from"../../middleware/utils";SwiperCore.use([Autoplay,Navigation,Thumbs,Pagination,Keyboard]);import dynamic from"next/dynamic";import useInView from"react-cool-inview";
const DynamicComponentWithCustomLoading = dynamic(
  () => import("../../components/haberComp"),
  { loading: () => <p>...</p> }
);
export default function BasketbolHaberleri({ basketbolKategori, profile }) {
 const{observe:observe,inView:inView}=useInView({onEnter:({unobserve:unobserve})=>unobserve()}),{observe:observe2,inView:inView2}=useInView({onEnter:({unobserve:unobserve2})=>unobserve2()}),link="https://kanaryasokagi.com/basketbol",baslik="Fenerbahçe Basketbol Haberleri - Kanaryasokağı",keywords="kanaryasokağı,fenerbahçe beko,fenerbahçe basketbol,basketbol süper ligi,basketbol süper lig,ıng basketbol ligi,beko basketbol ligi,beko fenerbahçe,süper lig basketbol,basketbol fenerbahçe,beko basketbol,ıng basketbol,fenerbahçe beko,basketbol,süper basketbol ligi,beko fenerbahçe basketbol,beko basketbol süper ligi,fenerbahçe basketbol kulübü,basketbol thy avrupa ligi,ıng basketbol süper ligi,basketbol kulübü,fenerbahçe beko süper lig,avrupa ligi basketbol,basketbol fenerbahçe beko,basketbol ligi fenerbahçe,basketbol beko,fenerbahçe basketbol avrupa ligi,fenerbahçe basketbol süper lig,fenerbahçe basketbol avrupa,fenerbahçe beko avrupa,thy fenerbahçe beko,fenerbahçe thy avrupa ligi,süper basketbol",[thumbsSwiper,setThumbsSwiper]=useState(null),{status:status,user:user}=useContext(AuthContext);useEffect(()=>{let leg;void 0!==profile.email?(user.setUserDetails({id:profile.id,email:profile.email,username:profile.username,firstname:profile.firstname,lastname:profile.lastname,roleId:profile.roleId,profile:profile.profile,isBanned:profile.isBanned}),status.setLoggedIn(!0)):status.setLoggedIn(!1)},[profile]);
  return (
    <Layout>
      <Head>
        <title>{baslik}</title>
              ...
              <Grid.Row>
                <Grid.Column>
                  <div className="w-full">
                    ...
                    >
                      {basketbolKategori.data.slice(0, 5).map((haber) => (
              ...
              <Grid.Row>
                <Grid.Column computer={5} tablet={5} mobile={16}>
                  <div className="space-y-4">
                    {basketbolKategori.data.slice(5, 6).map((haber, index) => (
                      <div
                  ...
                  className="w-full shadow-xl cursor-pointer xxs:px-4 xs:px-4 sm:px-4 md:px-0 lg:px-0 xl:px-0 2xl:px-0">
                    ...
                    {basketbolKategori.data.slice(6, 10).map((haber, index) => (
                      <div
                        className="w-full shadow-xl cursor-pointer xxs:px-4 xs:px-4 sm:px-4 md:px-0 lg:px-0 xl:px-0 2xl:px-0"
                        key={index}
                      >
                ...
                <Grid centered style={{ marginTop: 4 }}>
                  <Grid.Column computer={5} tablet={5} mobile={16}>
                    <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(20, 21)}
                    />
                     <div
          className="w-full shadow-xl cursor-pointer xxs:px-4 xs:px-4 sm:px-4 md:px-0 lg:px-0 xl:px-0 2xl:px-0"
        >
                    <AdSense.Google
                      ...
                      <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(21, 25)}
                    />
                  </Grid.Column>
                  <Grid.Column computer={5} tablet={5} mobile={16}>
                    <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(25, 27)}
                    />
                     <div
          className="w-full shadow-xl cursor-pointer xxs:px-4 xs:px-4 sm:px-4 md:px-0 lg:px-0 xl:px-0 2xl:px-0"
        >
                      ...
                      <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(27, 30)}
                    />
                  </Grid.Column>
                  <Grid.Column computer={5} tablet={5} mobile={16}>
                    <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(30, 34)}
                    />
                    <div
          className="w-full shadow-xl cursor-pointer xxs:px-4 xs:px-4 sm:px-4 md:px-0 lg:px-0 xl:px-0 2xl:px-0"
        >
                    <AdSense.Google
                      ...
                      />
                       ...
                       <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(34, 35)}
                    />
                  </Grid.Column>
                </Grid>
              )}
            ...
            <div ref={observe2}>
              {inView2 && (
                <Grid centered style={{ marginTop: 4 }}>
                  <Grid.Column computer={5} tablet={5} mobile={16}>
                    <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(35, 40)}
                    />
                  </Grid.Column>
                  <Grid.Column computer={5} tablet={5} mobile={16}>
                    <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(40, 45)}
                    />
                  </Grid.Column>
                  <Grid.Column computer={5} tablet={5} mobile={16}>
                    <DynamicComponentWithCustomLoading
                      data={basketbolKategori.data.slice(45, 50)}
                    />
                  </Grid.Column>
                </Grid>
              )}
...
export async function getServerSideProps(context) {
  ...
  const basketbolKategoriRes = await fetch(
      "http://localhost:8080/api/haberler/getByHaberKategori?categoryId=1"
    ),
    basketbolKategori = await basketbolKategoriRes.json();
  return basketbolKategori
    ? {
        props: {
