import Layout from"../../../components/layout";import Image1 from"next/image";import{useRouter}from"next/router";import InnerHTML from 'dangerously-set-html-content';import AdSense from "react-adsense";import Link from"next/link";import Head from"next/head";import Script from"next/script";import fetch from"isomorphic-fetch";import{useContext,useState,useEffect}from"react";import{AuthContext}from"../../../contexts/AuthProvider";import{absoluteUrl,getAppCookies,verifyToken}from"../../../middleware/utils";import{Button,Card,Comment,Form,Grid,Header,Icon,Image,Label,Message,Popup}from"semantic-ui-react";import dynamic from"next/dynamic";import{WhatsappIcon,WhatsappShareButton}from"next-share";import useInView from"react-cool-inview";import{Swiper,SwiperSlide}from"swiper/react";import SwiperCore,{Autoplay,Navigation,Thumbs,Pagination,Keyboard}from"swiper";SwiperCore.use([Autoplay,Navigation,Thumbs,Pagination,Keyboard]);
const DynamicComponentWithCustomLoading = dynamic(
  () => import("../../../components/haberComp"),
  { loading: () => <p>...</p> }
);

function BasketbolErkekHaber({
  puanDurumu,
  erkekBasketbolcular,
  profile,
  h,
              ...
              {puanDurumu.data.map((maclar, index) => (
                <SwiperSlide key={index}>
                  <div className="bg-white h-20 transition duration-300 ease-in-out transform hover:scale-90">
                    ...
                    {index > 13 && <div className="w-full h-0.5 bg-red"></div>}
                ...
                >
                  {erkekBasketbolcular.data.map((maclar, index) => (
                    <SwiperSlide key={index}>
                      <Link
                        href={{
                          pathname: `https://kanaryasokagi.com/oyuncular/basketbol/erkek/${maclar.oyuncuId}`,
                        }}
                      >
                        <a
                          target="_blank"
                  ...
                  <Grid.Row>
                    <Grid.Column computer={8} tablet={8} mobile={16}>
                      <div className="space-y-4">
                        ...
                        className="w-full bg-blue shadow-xl cursor-pointer xxs:px-4 xs:px-4 sm:px-4 md:px-0 lg:px-0 xl:px-0 2xl:px-0"
                      >
                         <AdSense.Google
                          client="ca-pub-2507406732896466"
                          slot="9220642681"
          ...
          <div ref={observe2}>
            {inView2 && (
              <Grid centered style={{ marginTop: 4 }}>
                <Grid.Column computer={8} tablet={8} mobile={16}>
                  <DynamicComponentWithCustomLoading
                    data={haberler2.slice(20, 25)}
                  />
                </Grid.Column>
                <Grid.Column computer={8} tablet={8} mobile={16}>
                  <DynamicComponentWithCustomLoading
                    data={haberler2.slice(25, 30)}
                  />
                </Grid.Column>
              </Grid>
            )}
...
export async function getServerSideProps(context) {
  ...
  const [
      haberlerRes,
      puanDurumuRes,
      erkekBasketbolcularRes,
      hRes,
      yorumlarRes,
    ] = await Promise.all([
      fetch(
        "http://localhost:8080/api/haberler/getByHaberKategoriAndCinsiyet?categoryId=1&cinsiyetId=0"
      ),
      fetch(
    ...
    [haberler, puanDurumu, erkekBasketbolcular, h, yorumData] =
      await Promise.all([
        haberlerRes.json(),
        puanDurumuRes.json(),
...
export default BasketbolErkekHaber;
