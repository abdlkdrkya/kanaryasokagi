import Layout from"../../../components/layout";import Image1 from"next/image";import{useRouter}from"next/router";import Link from"next/link";import InnerHTML from 'dangerously-set-html-content';import AdSense from "react-adsense";import Script from"next/script";import Head from"next/head";import{useContext,useState,useEffect}from"react";import fetch from"isomorphic-fetch";import{Image,Button,Comment,Form,Header,Icon,Label,Message,Popup,Grid,Card}from"semantic-ui-react";import{AuthContext}from"../../../contexts/AuthProvider";import{absoluteUrl,getAppCookies,verifyToken}from"../../../middleware/utils";import dynamic from"next/dynamic";import{WhatsappIcon,WhatsappShareButton}from"next-share";import{Swiper,SwiperSlide}from"swiper/react";import SwiperCore,{Autoplay,Navigation,Thumbs,Pagination,Keyboard}from"swiper";import useInView from"react-cool-inview";SwiperCore.use([Autoplay,Navigation,Thumbs,Pagination,Keyboard]);
const DynamicComponentWithCustomLoading = dynamic(
  () => import("../../../components/haberComp"),
  { loading: () => <p>...</p> }
);
function ErkekVoleybolHaber({
  haberler2,
  puanDurumu,
  erkekFutbolcular,
  profile,
              ...
              {puanDurumu.data.map((maclar, index) => (
                <SwiperSlide key={index}>
                  <div className="bg-white h-20 transition duration-300 ease-in-out transform hover:scale-90">
                    {index < 4 && <div className="w-full h-0.5 bg-green"></div>}
                    {index > 3 && index < 8 && (
                  ...
                  <Grid.Row>
                    ...
                    >
                      ...
                      >
                        {erkekFutbolcular.data.map((maclar, index) => (
                          <SwiperSlide key={index}>
                                <div className="bg-white mt-2 p-4 w-full transition duration-300 ease-in-out transform hover:scale-90 cursor-default">
                                  <Grid>
                                    <Grid.Row>
                                      <Grid.Column width={1}>
                                      ...
                                      <Grid.Column width={1}>
                                        <Image1
                                          ...
                                          height={56}
                                          src={maclar.oyuncuProfil}
                                          className="object-contain rounded-full cursor-default"
                                          alt={maclar.takim}
                                        />
                                      </Grid.Column>
                                      <Grid.Column width={8}>
                                        <span className="flex cursor-default text-xs sm:text-base md:text-base lg:text-base xl:text-base font-bold cursor-default">
                                          {maclar.oyuncuAdi}{" "}
                                          {maclar.oyuncuSoyAdi}
                                        </span>
                                      </Grid.Column>
...
const{req:req,query:query}=context,{origin:origin}=absoluteUrl(req),id=query.haberId,baseApiUrl="/api",{token:token}=getAppCookies(req),profile=token?verifyToken(token.split(" ")[1]):"";
  const [haberlerRes, puanDurumuRes, erkekFutbolcularRes, hRes, yorumlarRes] =
      await Promise.all([
        fetch(
          "http://localhost:8080/api/haberler/getByHaberKategoriAndCinsiyet?categoryId=3&cinsiyetId=0"
        ),
        fetch(
          "http://localhost:8080/api/puandurumu/voleybol-erkek-puandurumu-getir"
        ),
        fetch("http://localhost:8080/api/oyuncular/voleybol-erkek-getir"),
        fetch(
    ...
    [haberler, puanDurumu, erkekFutbolcular, h, yorumData] = await Promise.all([
      haberlerRes.json(),
      puanDurumuRes.json(),
...
export default ErkekVoleybolHaber;
